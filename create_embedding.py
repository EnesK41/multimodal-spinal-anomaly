...
def apply_augmentations(img_tensor, mask_tensor):
    # DİKKAT: Modelin esnekliği için hafif esnetme (+/- 10 derece) geri getirildi!
...
def generate_offline_dataset(num_aug_per_image=5):
    ...
    for index, row in df.iterrows():
        ...
        try:
            ...
            # Squeeze ile NIfTI'nin 3D (X, Y, Z) formatini PyTorch'un sevecegi (H, W) formatina basiyoruz
            xray_data = np.squeeze(xray_nii.get_fdata())
            affine = xray_nii.affine
            
            # Maske kontrolu: Yoksa bos (sifir) maske olustur
            if os.path.exists(mask_path):
                mask_nii = nib.load(mask_path)
                mask_data = np.squeeze(mask_nii.get_fdata())
            else:
                mask_data = np.zeros_like(xray_data)
                print(f"  -> BILGI: {patient_id} maskesi yok. Saglikli kabul edilip bos maske uretildi.")

            # (H, W) olan matrisi PyTorch'a uygun [1, H, W] formatina getiriyoruz
            ...
            print(f"      L-> {num_aug_per_image} adet cogaltilmis versiyon uretildi.")
    ...
    print("\n--- ISLEM TAMAM ---")
    print(f"Toplam {len(new_csv_data)} egitim verisi hazirlandi.")
    print(f"Yeni CSV dosyasi: {new_csv_path}")
