import argparse
import json
import os

import numpy as np
import torch
import torch.nn as nn
import torch.nn.functional as F
import torch.optim as optim
from torch.utils.data import DataLoader, Subset
from tqdm import tqdm

from dataset import SpineMultimodalDataset, print_dataset_stats
from models.XrayEncoder import XrayEncoder
from train import (
    aggregate_patient_level,
    build_patient_split,
    collect_patient_labels,
    compute_binary_metrics,
    find_best_threshold,
    make_stratified_patient_folds,
    print_metrics,
    print_threshold_sweep,
    save_experiment_results,
    set_global_seed,
    summarize_patient_split,
)
from train_multimodal_alignment import SimpleCTProjectionDataset, init_identity_embedding_head


REGION_NAMES = ["cervical", "thoracic", "lumbar"]
TYPE_NAMES = ["hemivertebra", "block vertebra", "butterfly vertebra"]


def add_aux_labels(sample):
    text = sample.get("text", "")
    lowered = text.lower() if isinstance(text, str) else ""

    region = torch.tensor(
        [1.0 if name in lowered else 0.0 for name in REGION_NAMES],
        dtype=torch.float32,
    )
    anomaly_type = torch.tensor(
        [1.0 if name in lowered else 0.0 for name in TYPE_NAMES],
        dtype=torch.float32,
    )
    is_positive = sample["label"].item() > 0.5
    sample["region"] = region
    sample["region_known"] = torch.tensor(
        1.0 if is_positive and region.sum().item() > 0 else 0.0,
        dtype=torch.float32,
    )
    sample["anomaly_type"] = anomaly_type
    sample["type_known"] = torch.tensor(
        1.0 if is_positive and anomaly_type.sum().item() > 0 else 0.0,
        dtype=torch.float32,
    )
    sample["text"] = "Spinal x-ray."
    return sample


class AuxLabelDataset(SpineMultimodalDataset):
    def __getitem__(self, idx):
        return add_aux_labels(super().__getitem__(idx))


class AuxCTLabelDataset(SimpleCTProjectionDataset):
    def __getitem__(self, idx):
        return add_aux_labels(super().__getitem__(idx))


class AuxiliaryMultiTaskHead(nn.Module):
    def __init__(self, embedding_dim=512, dropout=0.5, use_region=True, use_type=False):
        super().__init__()
        self.use_region = use_region
        self.use_type = use_type
        self.shared = nn.Sequential(
            nn.LayerNorm(embedding_dim),
            nn.Linear(embedding_dim, 256),
            nn.ReLU(inplace=True),
            nn.Dropout(dropout),
        )
        self.anomaly_head = nn.Linear(256, 1)
        self.region_head = nn.Linear(256, len(REGION_NAMES)) if use_region else None
        self.type_head = nn.Linear(256, len(TYPE_NAMES)) if use_type else None

    def forward(self, embedding):
        features = self.shared(embedding)
        anomaly_logits = self.anomaly_head(features).squeeze(1)
        region_logits = self.region_head(features) if self.region_head is not None else None
        type_logits = self.type_head(features) if self.type_head is not None else None
        return anomaly_logits, region_logits, type_logits


def make_dataset(args):
    common = dict(
        data_dir=args.data_dir,
        csv_file=args.csv_file,
        img_size=(1024, 512),
        is_train=True,
        exclude_bad_patients=True,
        use_body_roi=True,
        roi_width_ratio=args.roi_width_ratio,
        scrub_prompt=False,
        label_source="mask",
    )
    if args.use_ct_alignment:
        return AuxCTLabelDataset(
            **common,
            original_patients_dir=args.original_patients_dir,
            ct_cache_dir=args.ct_cache_dir,
            ct_projection_axis=args.ct_projection_axis,
            ct_projection_method=args.ct_projection_method,
            ct_projection_source=args.ct_projection_source,
            ct_hu_threshold=args.ct_hu_threshold,
            ct_clip_max=args.ct_clip_max,
            ct_rotate_k=args.ct_rotate_k,
        )
    return AuxLabelDataset(**common)


def count_aux_labels(dataset, indices, title):
    region_totals = np.zeros(len(REGION_NAMES), dtype=np.int64)
    type_totals = np.zeros(len(TYPE_NAMES), dtype=np.int64)
    region_known = 0
    type_known = 0
    positive = 0
    for idx in indices:
        sample = dataset[idx]
        if sample["label"].item() > 0.5:
            positive += 1
        if sample["region_known"].item() > 0.5:
            region_known += 1
            region_totals += sample["region"].numpy().astype(np.int64)
        if sample["type_known"].item() > 0.5:
            type_known += 1
            type_totals += sample["anomaly_type"].numpy().astype(np.int64)

    print("=" * 100)
    print(f"{title} AUXILIARY LABELS")
    print("=" * 100)
    print(f"Positive samples             : {positive}")
    print(f"Positive samples with region : {region_known}/{positive}")
    for name, value in zip(REGION_NAMES, region_totals):
        print(f"region {name:16s}: {int(value)}")
    print(f"Positive samples with type   : {type_known}/{positive}")
    for name, value in zip(TYPE_NAMES, type_totals):
        print(f"type   {name:16s}: {int(value)}")
    print("=" * 100)


def compute_pos_weight(dataset, indices, label_key, known_key, names):
    pos = np.zeros(len(names), dtype=np.float32)
    known = 0.0
    for idx in indices:
        sample = dataset[idx]
        if sample[known_key].item() > 0.5:
            known += 1.0
            pos += sample[label_key].numpy().astype(np.float32)
    neg = np.maximum(known - pos, 0.0)
    return neg / np.maximum(pos, 1.0)


def compute_multilabel_metrics(targets, probs, known_mask, names, threshold=0.5):
    targets = np.asarray(targets, dtype=np.float32)
    probs = np.asarray(probs, dtype=np.float32)
    known = np.asarray(known_mask, dtype=np.float32) > 0.5

    if targets.size == 0 or known.sum() == 0:
        return {"n_known": 0, "exact_match": None, "macro_f1": None, "per_label": {}}

    targets = targets[known]
    probs = probs[known]
    preds = (probs >= threshold).astype(np.float32)

    exact = float((preds == targets).all(axis=1).mean())
    per_label = {}
    f1s = []
    for i, name in enumerate(names):
        y = targets[:, i]
        p = preds[:, i]
        tp = int(((y == 1) & (p == 1)).sum())
        tn = int(((y == 0) & (p == 0)).sum())
        fp = int(((y == 0) & (p == 1)).sum())
        fn = int(((y == 1) & (p == 0)).sum())
        precision = tp / max(tp + fp, 1)
        recall = tp / max(tp + fn, 1)
        f1 = (2 * precision * recall) / max(precision + recall, 1e-8)
        f1s.append(f1)
        per_label[name] = {
            "precision": precision,
            "recall": recall,
            "f1": f1,
            "tp": tp,
            "tn": tn,
            "fp": fp,
            "fn": fn,
        }

    return {
        "n_known": int(known.sum()),
        "exact_match": exact,
        "macro_f1": float(sum(f1s) / len(f1s)),
        "per_label": per_label,
    }


def print_multilabel_metrics(title, metrics):
    print("-" * 100)
    print(title)
    print("-" * 100)
    print(f"Known positives : {metrics['n_known']}")
    if metrics["exact_match"] is None:
        print("Metrics         : N/A")
    else:
        print(f"Exact match     : {metrics['exact_match']:.6f}")
        print(f"Macro F1        : {metrics['macro_f1']:.6f}")
        for name, row in metrics["per_label"].items():
            print(
                f"{name:18s} | f1={row['f1']:.6f} | "
                f"precision={row['precision']:.6f} | recall={row['recall']:.6f} | "
                f"tp/tn/fp/fn={row['tp']}/{row['tn']}/{row['fp']}/{row['fn']}"
            )
    print("-" * 100)


def masked_bce_loss(loss_fn, logits, targets, known):
    if logits is None or known.sum().item() <= 0:
        return targets.new_zeros(())
    per_sample = loss_fn(logits, targets).mean(dim=1)
    return (per_sample * known).sum() / known.sum().clamp_min(1.0)


def run_one_fold(args, dataset, fold_idx, val_patients):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    suffixes = []
    if args.use_region:
        suffixes.append("region")
    if args.use_type:
        suffixes.append("type")
    if args.use_ct_alignment:
        suffixes.append("ctmask")
    suffix = "_".join(suffixes) or "anomaly"
    run_name = (
        f"aux_{suffix}_roi{str(args.roi_width_ratio).replace('.', '')}_"
        f"fold_{fold_idx:02d}_seed{args.seed}"
    )
    checkpoint_path = os.path.join("checkpoints", f"{run_name}.pth")
    os.makedirs("checkpoints", exist_ok=True)

    train_idx, val_idx = build_patient_split(
        dataset=dataset,
        val_patients=val_patients,
        original_only_val=not args.include_val_augmentations,
    )
    if not val_idx:
        raise RuntimeError("Validation split is empty.")

    train_pos = 0
    train_neg = 0
    for idx in train_idx:
        label = dataset[idx]["label"].item()
        if label > 0.5:
            train_pos += 1
        else:
            train_neg += 1

    count_aux_labels(dataset, train_idx, "TRAIN")
    count_aux_labels(dataset, val_idx, "VAL")

    train_loader = DataLoader(
        Subset(dataset, train_idx),
        batch_size=args.batch_size,
        shuffle=True,
        num_workers=args.num_workers,
        pin_memory=True,
    )
    val_loader = DataLoader(
        Subset(dataset, val_idx),
        batch_size=1,
        shuffle=False,
        num_workers=args.num_workers,
        pin_memory=True,
    )

    xray_enc = XrayEncoder(embedding_dim=512, pretrained=args.pretrained_encoder).to(device)
    head = AuxiliaryMultiTaskHead(
        embedding_dim=512,
        dropout=args.dropout,
        use_region=args.use_region,
        use_type=args.use_type,
    ).to(device)

    ct_enc = None
    if args.use_ct_alignment:
        ct_enc = XrayEncoder(embedding_dim=512, pretrained=args.pretrained_encoder).to(device)
        if args.identity_embedding_head:
            init_identity_embedding_head(ct_enc)
        for param in ct_enc.parameters():
            param.requires_grad = False
        ct_enc.eval()

    optimizer_params = [
        {"params": xray_enc.parameters(), "lr": args.encoder_lr},
        {"params": head.parameters(), "lr": args.head_lr},
    ]
    optimizer = optim.AdamW(optimizer_params, weight_decay=args.weight_decay)
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(
        optimizer,
        mode="max",
        factor=0.5,
        patience=2,
    )

    pos_weight_val = train_neg / max(train_pos, 1) if train_pos > 0 else 1.0
    anomaly_loss_fn = nn.BCEWithLogitsLoss(
        pos_weight=torch.tensor([pos_weight_val], dtype=torch.float32, device=device)
    )

    region_pos_weight = None
    if args.balance_region_loss:
        region_pos_weight = torch.tensor(
            compute_pos_weight(dataset, train_idx, "region", "region_known", REGION_NAMES),
            dtype=torch.float32,
            device=device,
        )
    type_pos_weight = None
    if args.balance_type_loss:
        type_pos_weight = torch.tensor(
            compute_pos_weight(dataset, train_idx, "anomaly_type", "type_known", TYPE_NAMES),
            dtype=torch.float32,
            device=device,
        )

    region_loss_fn = nn.BCEWithLogitsLoss(reduction="none", pos_weight=region_pos_weight)
    type_loss_fn = nn.BCEWithLogitsLoss(reduction="none", pos_weight=type_pos_weight)

    print("=" * 100)
    print("AUXILIARY MULTITASK TRAINING")
    print("=" * 100)
    print(f"Run name             : {run_name}")
    print(f"Device               : {device}")
    print(f"Use region head      : {args.use_region}")
    print(f"Use type head        : {args.use_type}")
    print(f"Use CT alignment     : {args.use_ct_alignment}")
    print(f"Region/type/align w  : {args.region_loss_weight} / {args.type_loss_weight} / {args.alignment_weight}")
    print(f"BCE pos_weight       : {pos_weight_val:.6f}")
    print(f"Checkpoint           : {checkpoint_path}")
    print("=" * 100)

    best_score = -1.0
    best_epoch = -1
    best_threshold = 0.5
    patience = 0
    best_payload = None

    for epoch in range(args.epochs):
        print("\n" + "=" * 100)
        print(f"EPOCH {epoch + 1}/{args.epochs} | {run_name}")
        print("=" * 100)

        xray_enc.train()
        head.train()
        if ct_enc is not None:
            ct_enc.eval()

        train_labels = []
        train_probs = []
        train_patient_ids = []
        train_region_targets = []
        train_region_probs = []
        train_region_known = []
        train_type_targets = []
        train_type_probs = []
        train_type_known = []
        align_cos_values = []

        total_loss = 0.0
        total_anomaly_loss = 0.0
        total_region_loss = 0.0
        total_type_loss = 0.0
        total_align_loss = 0.0

        pbar = tqdm(train_loader, desc=f"Training {run_name}")
        for batch in pbar:
            xray = batch["xray"].to(device, non_blocking=True)
            labels = batch["label"].to(device, non_blocking=True).float()
            regions = batch["region"].to(device, non_blocking=True).float()
            region_known = batch["region_known"].to(device, non_blocking=True).float()
            anomaly_types = batch["anomaly_type"].to(device, non_blocking=True).float()
            type_known = batch["type_known"].to(device, non_blocking=True).float()

            optimizer.zero_grad(set_to_none=True)
            xray_emb, _ = xray_enc(xray)
            anomaly_logits, region_logits, type_logits = head(xray_emb)

            anomaly_loss = anomaly_loss_fn(anomaly_logits.view(-1), labels)
            region_loss = masked_bce_loss(region_loss_fn, region_logits, regions, region_known)
            type_loss = masked_bce_loss(type_loss_fn, type_logits, anomaly_types, type_known)

            align_loss = torch.zeros((), device=device)
            if args.use_ct_alignment:
                ct_projection = batch["ct_projection"].to(device, non_blocking=True)
                has_ct = batch["has_ct"].to(device, non_blocking=True).float()
                if has_ct.sum().item() > 0:
                    with torch.no_grad():
                        ct_emb, _ = ct_enc(ct_projection)
                    mask = has_ct > 0.5
                    x_norm = F.normalize(xray_emb[mask], dim=1)
                    ct_norm = F.normalize(ct_emb[mask], dim=1)
                    cos = (x_norm * ct_norm).sum(dim=1)
                    align_loss = (1.0 - cos).mean()
                    align_cos_values.extend(cos.detach().cpu().tolist())

            loss = (
                anomaly_loss
                + args.region_loss_weight * region_loss
                + args.type_loss_weight * type_loss
                + args.alignment_weight * align_loss
            )
            loss.backward()
            grad_norm = torch.nn.utils.clip_grad_norm_(
                list(xray_enc.parameters()) + list(head.parameters()),
                max_norm=1.0,
            )
            optimizer.step()

            probs = torch.sigmoid(anomaly_logits.detach())
            region_probs = torch.sigmoid(region_logits.detach()) if region_logits is not None else None
            type_probs = torch.sigmoid(type_logits.detach()) if type_logits is not None else None

            total_loss += loss.item()
            total_anomaly_loss += anomaly_loss.item()
            total_region_loss += region_loss.item()
            total_type_loss += type_loss.item()
            total_align_loss += align_loss.item()

            train_labels.extend(labels.detach().cpu().tolist())
            train_probs.extend(probs.detach().cpu().tolist())
            train_patient_ids.extend(batch["base_patient_id"])
            train_region_targets.extend(regions.detach().cpu().tolist())
            train_region_known.extend(region_known.detach().cpu().tolist())
            train_type_targets.extend(anomaly_types.detach().cpu().tolist())
            train_type_known.extend(type_known.detach().cpu().tolist())
            if region_probs is not None:
                train_region_probs.extend(region_probs.cpu().tolist())
            if type_probs is not None:
                train_type_probs.extend(type_probs.cpu().tolist())

            pbar.set_postfix(
                {
                    "loss": f"{loss.item():.4f}",
                    "cls": f"{anomaly_loss.item():.4f}",
                    "region": f"{region_loss.item():.4f}",
                    "type": f"{type_loss.item():.4f}",
                    "align": f"{align_loss.item():.4f}",
                    "grad": f"{float(grad_norm):.3f}",
                }
            )

        _, train_patient_labels, train_patient_probs = aggregate_patient_level(
            train_patient_ids,
            train_labels,
            train_probs,
            mode="mean",
        )
        train_metrics = compute_binary_metrics(train_patient_labels, train_patient_probs, threshold=0.5)
        print_metrics("TRAIN PATIENT-LEVEL METRICS | MEAN AGG | DEFAULT THRESHOLD", train_metrics)
        print(
            "Train loss total/class/region/type/align: "
            f"{total_loss:.6f} / {total_anomaly_loss:.6f} / {total_region_loss:.6f} / "
            f"{total_type_loss:.6f} / {total_align_loss:.6f}"
        )
        if align_cos_values:
            print(f"Train paired CT cosine mean/std: {np.mean(align_cos_values):.6f} / {np.std(align_cos_values):.6f}")

        xray_enc.eval()
        head.eval()

        val_labels = []
        val_probs = []
        val_patient_ids = []
        val_region_targets = []
        val_region_probs = []
        val_region_known = []
        val_type_targets = []
        val_type_probs = []
        val_type_known = []
        val_rows = []
        val_align_cos_values = []

        with torch.no_grad():
            for batch in val_loader:
                xray = batch["xray"].to(device, non_blocking=True)
                labels = batch["label"].to(device, non_blocking=True).float()
                regions = batch["region"].to(device, non_blocking=True).float()
                region_known = batch["region_known"].to(device, non_blocking=True).float()
                anomaly_types = batch["anomaly_type"].to(device, non_blocking=True).float()
                type_known = batch["type_known"].to(device, non_blocking=True).float()

                xray_emb, _ = xray_enc(xray)
                anomaly_logits, region_logits, type_logits = head(xray_emb)
                probs = torch.sigmoid(anomaly_logits)
                region_probs = torch.sigmoid(region_logits) if region_logits is not None else torch.zeros_like(regions)
                type_probs = torch.sigmoid(type_logits) if type_logits is not None else torch.zeros_like(anomaly_types)

                align_cos = None
                if args.use_ct_alignment:
                    ct_projection = batch["ct_projection"].to(device, non_blocking=True)
                    has_ct = batch["has_ct"].to(device, non_blocking=True).float()
                    if has_ct.sum().item() > 0:
                        ct_emb, _ = ct_enc(ct_projection)
                        x_norm = F.normalize(xray_emb, dim=1)
                        ct_norm = F.normalize(ct_emb, dim=1)
                        align_cos = (x_norm * ct_norm).sum(dim=1).item()
                        val_align_cos_values.append(align_cos)

                label_val = labels.item()
                prob_val = probs.item()
                base_pid = batch["base_patient_id"][0]
                patient_id = batch["patient_id"][0]

                val_labels.append(label_val)
                val_probs.append(prob_val)
                val_patient_ids.append(base_pid)
                val_region_targets.extend(regions.cpu().tolist())
                val_region_probs.extend(region_probs.cpu().tolist())
                val_region_known.extend(region_known.cpu().tolist())
                val_type_targets.extend(anomaly_types.cpu().tolist())
                val_type_probs.extend(type_probs.cpu().tolist())
                val_type_known.extend(type_known.cpu().tolist())

                val_rows.append(
                    {
                        "patient_id": patient_id,
                        "base_patient_id": base_pid,
                        "label": label_val,
                        "prob": prob_val,
                        "region_known": float(region_known.item()),
                        "region_target": regions.squeeze(0).cpu().tolist(),
                        "region_prob": region_probs.squeeze(0).cpu().tolist(),
                        "type_known": float(type_known.item()),
                        "type_target": anomaly_types.squeeze(0).cpu().tolist(),
                        "type_prob": type_probs.squeeze(0).cpu().tolist(),
                        "align_cos": align_cos,
                    }
                )

        _, val_patient_labels, val_patient_probs = aggregate_patient_level(
            val_patient_ids,
            val_labels,
            val_probs,
            mode="mean",
        )
        default_metrics = compute_binary_metrics(val_patient_labels, val_patient_probs, threshold=0.5)
        best_t, best_metrics, threshold_rows = find_best_threshold(val_patient_labels, val_patient_probs)
        region_metrics = compute_multilabel_metrics(
            val_region_targets,
            val_region_probs,
            val_region_known,
            REGION_NAMES,
        )
        type_metrics = compute_multilabel_metrics(
            val_type_targets,
            val_type_probs,
            val_type_known,
            TYPE_NAMES,
        )
        scheduler.step(best_metrics["balanced_accuracy"])

        print_threshold_sweep(threshold_rows)
        print_metrics("VAL PATIENT-LEVEL METRICS | MEAN AGG | DEFAULT THRESHOLD", default_metrics)
        print_metrics(
            f"VAL PATIENT-LEVEL METRICS | MEAN AGG | BEST THRESHOLD={best_t:.3f}",
            best_metrics,
        )
        if args.use_region:
            print_multilabel_metrics("VAL REGION METRICS | POSITIVE SAMPLES ONLY", region_metrics)
        if args.use_type:
            print_multilabel_metrics("VAL TYPE METRICS | POSITIVE SAMPLES ONLY", type_metrics)
        if val_align_cos_values:
            print(f"Val paired CT cosine mean/std: {np.mean(val_align_cos_values):.6f} / {np.std(val_align_cos_values):.6f}")

        score = best_metrics["balanced_accuracy"]
        if epoch + 1 >= args.min_save_epoch and score > best_score:
            best_score = score
            best_epoch = epoch + 1
            best_threshold = best_t
            patience = 0
            best_payload = {
                "fold": fold_idx,
                "run_name": run_name,
                "val_patients": val_patients,
                "best_epoch": best_epoch,
                "best_score": best_score,
                "best_threshold": best_threshold,
                "default_threshold_metrics": default_metrics,
                "best_threshold_metrics": best_metrics,
                "region_metrics": region_metrics,
                "type_metrics": type_metrics,
                "val_rows": val_rows,
            }
            torch.save(
                {
                    "epoch": best_epoch,
                    "xray_enc": xray_enc.state_dict(),
                    "head": head.state_dict(),
                    "best_score": best_score,
                    "best_threshold": best_threshold,
                    "region_names": REGION_NAMES,
                    "type_names": TYPE_NAMES,
                    "args": vars(args),
                },
                checkpoint_path,
            )
            print(f"MODEL SAVED | best_score={best_score:.6f} | threshold={best_threshold:.3f}")
        elif epoch + 1 >= args.min_save_epoch:
            patience += 1
            print(f"MODEL NOT SAVED | patience={patience}/{args.early_stopping_patience}")

        if epoch + 1 >= args.min_save_epoch and patience >= args.early_stopping_patience:
            print("EARLY STOPPING")
            break

    if best_payload is None:
        best_payload = {
            "fold": fold_idx,
            "run_name": run_name,
            "val_patients": val_patients,
            "best_epoch": best_epoch,
            "best_score": best_score,
            "best_threshold": best_threshold,
        }

    best_payload["checkpoint"] = checkpoint_path
    return best_payload


def run_cv(args):
    set_global_seed(args.seed)
    dataset = make_dataset(args)
    print_dataset_stats(dataset)
    patient_labels = collect_patient_labels(dataset)
    folds = make_stratified_patient_folds(patient_labels, n_splits=args.folds, seed=args.seed)

    results = []
    for fold_idx, val_patients in enumerate(folds, start=1):
        summarize_patient_split(patient_labels, val_patients, f"AUXILIARY MULTITASK CV FOLD {fold_idx}/{args.folds}")
        result = run_one_fold(args, dataset, fold_idx, val_patients)
        results.append(result)

    scores = [row["best_score"] for row in results if row.get("best_score", -1) >= 0]
    region_exact = [
        row.get("region_metrics", {}).get("exact_match")
        for row in results
        if row.get("region_metrics", {}).get("exact_match") is not None
    ]
    region_macro_f1 = [
        row.get("region_metrics", {}).get("macro_f1")
        for row in results
        if row.get("region_metrics", {}).get("macro_f1") is not None
    ]
    type_exact = [
        row.get("type_metrics", {}).get("exact_match")
        for row in results
        if row.get("type_metrics", {}).get("exact_match") is not None
    ]
    type_macro_f1 = [
        row.get("type_metrics", {}).get("macro_f1")
        for row in results
        if row.get("type_metrics", {}).get("macro_f1") is not None
    ]

    payload = {
        "experiment": "auxiliary_multitask",
        "seed": args.seed,
        "folds": args.folds,
        "region_names": REGION_NAMES,
        "type_names": TYPE_NAMES,
        "mean_best_balanced_accuracy": sum(scores) / max(len(scores), 1),
        "mean_region_exact_match": sum(region_exact) / max(len(region_exact), 1) if region_exact else None,
        "mean_region_macro_f1": sum(region_macro_f1) / max(len(region_macro_f1), 1) if region_macro_f1 else None,
        "mean_type_exact_match": sum(type_exact) / max(len(type_exact), 1) if type_exact else None,
        "mean_type_macro_f1": sum(type_macro_f1) / max(len(type_macro_f1), 1) if type_macro_f1 else None,
        "args": vars(args),
        "results": results,
    }

    pieces = []
    if args.use_region:
        pieces.append("region")
    if args.use_type:
        pieces.append("type")
    if args.use_ct_alignment:
        pieces.append("ctmask")
    name = "_".join(pieces) or "anomaly"
    filename = (
        f"aux_{name}_roi{str(args.roi_width_ratio).replace('.', '')}_"
        f"rw{str(args.region_loss_weight).replace('.', '')}_"
        f"tw{str(args.type_loss_weight).replace('.', '')}_"
        f"aw{str(args.alignment_weight).replace('.', '')}_seed{args.seed}.json"
    )
    save_experiment_results(filename, payload)

    print("=" * 100)
    print("AUXILIARY MULTITASK CV SUMMARY")
    print("=" * 100)
    print(f"Mean best balanced accuracy: {payload['mean_best_balanced_accuracy']:.6f}")
    print(f"Mean region exact/macro F1 : {payload['mean_region_exact_match']} / {payload['mean_region_macro_f1']}")
    print(f"Mean type exact/macro F1   : {payload['mean_type_exact_match']} / {payload['mean_type_macro_f1']}")
    for row in results:
        print(
            f"Fold {row['fold']}: best_score={row.get('best_score', -1):.6f} | "
            f"epoch={row.get('best_epoch', -1)} | threshold={row.get('best_threshold', 0.5):.3f}"
        )
    print("=" * 100)


def parse_args():
    parser = argparse.ArgumentParser(description="Auxiliary multitask X-ray classifier")
    parser.add_argument("--data_dir", default="data/augmented_patients")
    parser.add_argument("--csv_file", default="data/train_labels_augmented.csv")
    parser.add_argument("--original_patients_dir", default="data/original_patients")
    parser.add_argument("--ct_cache_dir", default="data/ct_projections_simple")
    parser.add_argument("--epochs", type=int, default=5)
    parser.add_argument("--batch_size", type=int, default=8)
    parser.add_argument("--folds", type=int, default=5)
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--roi_width_ratio", type=float, default=0.45)
    parser.add_argument("--region_loss_weight", type=float, default=0.2)
    parser.add_argument("--type_loss_weight", type=float, default=0.1)
    parser.add_argument("--alignment_weight", type=float, default=0.1)
    parser.add_argument("--encoder_lr", type=float, default=1e-5)
    parser.add_argument("--head_lr", type=float, default=5e-5)
    parser.add_argument("--weight_decay", type=float, default=5e-4)
    parser.add_argument("--dropout", type=float, default=0.5)
    parser.add_argument("--min_save_epoch", type=int, default=1)
    parser.add_argument("--early_stopping_patience", type=int, default=3)
    parser.add_argument("--num_workers", type=int, default=0)
    parser.add_argument("--pretrained_encoder", action="store_true")
    parser.add_argument("--identity_embedding_head", action="store_true")
    parser.add_argument("--include_val_augmentations", action="store_true")
    parser.add_argument("--use_region", action="store_true")
    parser.add_argument("--use_type", action="store_true")
    parser.add_argument("--use_ct_alignment", action="store_true")
    parser.add_argument("--balance_region_loss", action="store_true")
    parser.add_argument("--balance_type_loss", action="store_true")
    parser.add_argument("--ct_projection_axis", type=int, default=1)
    parser.add_argument("--ct_projection_method", choices=["max", "mean"], default="max")
    parser.add_argument("--ct_projection_source", choices=["ct", "ct_mask", "masked_ct"], default="ct_mask")
    parser.add_argument("--ct_hu_threshold", type=float, default=None)
    parser.add_argument("--ct_clip_max", type=float, default=1800.0)
    parser.add_argument("--ct_rotate_k", type=int, default=1)
    return parser.parse_args()


if __name__ == "__main__":
    run_cv(parse_args())
